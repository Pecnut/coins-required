...
data = []
item_ids = []; item_descs = []; validities = []; shop_codes = []; all_prices = []; regions = []; shop_weights = [];
...
indices_item_ids = []; indices_cpi_weight = [];
...
# CPI weight is amount of income spent on this thing (*1000) but I want how common it is to buy itself.
...
# Pick random price for each item, weighted on shop weight
'''
print "Selecting a price for each item..."
item_ids_unique = list(set(item_ids))
random_pick_prices = []
for item_id in item_ids_unique:
    prices_for_that_item = [prices[i] for i in xrange(len(prices)) if item_ids[i] == item_id]
            ...
            for basket_item_number in xrange(basket_size):
                ...
                while not picked_item:
                    ...
                    try:
                        ...
                        #item_picked_price = prices[item_picked_id_in_prices_list]
                        # End test
